...
def get_hits(vec, test_pair, top_k=(1, 5, 10, 50, 100)):
    Lvec = np.array([vec[e1] for e1, e2 in test_pair])
    Rvec = np.array([vec[e2] for e1, e2 in test_pair])
    sim = scipy.spatial.distance.cdist(Lvec, Rvec, metric='cityblock')  #[ [dis(0,0), dis(0,1), , , ], [dis(1,0), dis(1,1), , , ] ]
    MR = {'L_R':0, 'R_L':0}
    MRR = {'L_R':0.0, 'R_L':0.0}
    top_lr = [0] * len(top_k)
    for i in range(Lvec.shape[0]):
        rank = sim[i, :].argsort()  #在第i个数组（维）中取全部数据
...
def get_combine_hits(se_vec, ae_vec, beta, test_pair, top_k=(1, 5, 10, 50, 100)):
